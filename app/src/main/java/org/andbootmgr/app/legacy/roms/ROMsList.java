...
public class ROMsList {
    ...
    public List<ROM> getROMs() {
        ...
        for (String sn : Objects.requireNonNull(SuFile.open("/data/data/org.andbootmgr.app/assets/Scripts/add_os/" + codename + "/").list())) {
            ROM r = new ROM();
            r.scriptname = sn;
            r.fullPath = "/data/data/org.andbootmgr.app/assets/Scripts/add_os/" + codename + "/" + r.scriptname;
            ...
            switch (r.scriptname) {
                case "add_ubuntutouch_systemimage_haliumboot.sh":
                    ...
                    r.gen = (imodel, menuName, folderName) -> imodel.setCmdline(Objects.requireNonNull(imodel.getROM().getValue()).fullPath + " '" + folderName + "' '" + menuName + "' " + Objects.requireNonNull(imodel.getParts().getValue()).get(0) + " " + imodel.getParts().getValue().get(1) + " /data/data/org.andbootmgr.app/cache/halium-boot.img");
                    break;
                case "add_droidian.sh":
                    ...
                    r.strings.put(c.getString(R.string.enter_rom_folder), "rom" + b);
                    r.gen = (imodel, menuName, folderName) -> imodel.setCmdline(Objects.requireNonNull(imodel.getROM().getValue()).fullPath + " '" + folderName + "' '" + menuName + "' " + Objects.requireNonNull(imodel.getParts().getValue()).get(0) + " /data/data/org.andbootmgr.app/cache/droidian-gsi.zip /data/data/org.andbootmgr.app/cache/droidian-devtools.zip /data/data/org.andbootmgr.app/cache/droidian-adapt.zip");
                    break;
                case "add_sailfish.sh":
                    ...
                    r.strings.put(c.getString(R.string.enter_rom_folder), "rom" + b);
                    r.gen = (imodel, menuName, folderName) -> imodel.setCmdline(Objects.requireNonNull(imodel.getROM().getValue()).fullPath + " '" + folderName + "' '" + menuName + "' " + Objects.requireNonNull(imodel.getParts().getValue()).get(0) + " /data/data/org.andbootmgr.app/cache/hybris-boot.img");
                    break;
                case "other_os.sh":
                    ...
                    r.strings.put(c.getString(R.string.enter_rom_folder), "rom" + b);
                    r.gen = (imodel, menuName, folderName) -> imodel.setCmdline(Objects.requireNonNull(imodel.getROM().getValue()).fullPath + " '" + folderName + "' '" + menuName + "' /data/data/org.andbootmgr.app/cache/boot.img");
                    break;
                case "entry_only.sh":
